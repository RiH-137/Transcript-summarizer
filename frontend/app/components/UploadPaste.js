...
export default function UploadPaste({ onTextChange }) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
      <label className="block text-sm font-medium text-gray-700 mb-4">Upload Document or Audio File</label>
      
      {/* Upload Area */}
      <div 
        className={`border-2 border-dashed rounded-lg p-8 text-center transition-all duration-200 cursor-pointer ${
          isUploading 
            ? 'border-[#417C7E] bg-[#417C7E]/5' 
            : 'border-gray-300 hover:border-[#417C7E] hover:bg-[#417C7E]/5'
        }`}
      ...
      >
        ...
        <div className="space-y-3">
          ...
          {!isUploading && (
            <button 
              type="button"
              className="px-4 py-2 bg-[#417C7E] text-white rounded-lg hover:bg-[#2d5759] transition-colors"
            >
              Choose File
            </button>
          )}
      ...
      {isUploading && (
        <div className="mt-4">
          <div className="bg-gray-200 rounded-full h-2">
            <div className="bg-[#417C7E] h-2 rounded-full animate-pulse" style={{width: '100%'}}></div>
          </div>
